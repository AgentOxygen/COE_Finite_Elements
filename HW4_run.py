...
class FiniteElement2D:
    ...
    def genLocal2DTrussStiffnessMatrix(self, n1_pos: np.array, n2_pos: np.array, youngs: float, area: float) -> np.array:
        """
    ...
    def buildGlobalStiffnessMatrix(self, pos: dict, conns: dict, global_index: dict) -> np.array:
        ...
        for conn in conns:
            ...
            local_k = self.genLocal2DTrussStiffnessMatrix(n1_pos, n2_pos, conn["youngs_mod"], conn["area"])
    ...
    def calculateLengthDeltaAndStrains(self, element_index: dict, input_structure: dict, output_structure: dict) -> tuple:
        ...
        for index in element_index:
            ...
            x1, y1 = input_structure["node_positions"][na]
            x2, y2 = input_structure["node_positions"][nb]
            l1 = pow(pow(x2-x1, 2) + pow(y2-y1, 2), 0.5)
            x1, y1 = output_structure["node_positions"][na]
            x2, y2 = output_structure["node_positions"][nb]
...
if __name__ == "__main__":
    ...
    analysis = FiniteElement2D("HW4_bridge.json")
    ...
    for node in analysis.input_structure["external_node_forces"]:
        x, y = analysis.input_structure["node_positions"][node]
        fu, fv = analysis.input_structure["external_node_forces"][node]
        if put_label:
            ax1.arrow(x, y, fu, fv, width=0.25, color="Blue", label="External Force", length_includes_head=True, head_length=0.3)
            put_label = False
        else:
            ax1.arrow(x, y, fu, fv, width=0.25, color="Blue", length_includes_head=True, head_length=0.3)

    # Add points and connections
    for data, color, ls, lw, label in [(analysis.input_structure, "Green", "-", 4, "Initial"), ((analysis.output_structure, "Red", "--", 3, "Final"))]:
        pts_x = []
        pts_y = []
        pts_labels = []
        ...
        for conn in data["node_connections"]:
            a, b = conn["labels"]
            x1, y1 = data["node_positions"][a]
            x2, y2 = data["node_positions"][b]
            if put_label:
        ...
        for i, label in enumerate(pts_labels):
            ax1.annotate(label, (pts_x[i], pts_y[i]), color="Black", fontsize=20)

    ax1.grid()
    ax1.legend()

    index = np.argmax(analysis.element_internal_forces)
    print(f"Max tension in element {analysis.element_index[index]}: {analysis.element_internal_forces[index]} P")
    print(f"{analysis.element_index[index]} Stress: {analysis.element_stresses[index]} P/A")
    print(f"{analysis.element_index[index]} Strain: {analysis.element_strains[index]}\n")

    index = np.argmin(analysis.element_internal_forces)
    print(f"Max compression in element {analysis.element_index[index]}: {analysis.element_internal_forces[index]} P")
    print(f"{analysis.element_index[index]} Stress: {analysis.element_stresses[index]} P/A")
    print(f"{analysis.element_index[index]} Strain: {analysis.element_strains[index]}\n")

    print("See 'HW4_deform.png' for reference")
    f.savefig("HW4_deform.png")